...
- 🔒 نظام حماية JWT كامل (مصادقة + صلاحيات)
- ⚡ حماية Rate Limiting لتسجيل دخول الأدمن
...
│       └── api.ts             # إدارة JWT tokens + fetch مع مصادقة
│
...
جميع الـ API routes محمية بنظام JWT:

- عند تسجيل الدخول (مستخدم أو أدمن)، السيرفر يرجع **JWT token** صالح لـ 7 أيام
- الفرونت اند يخزن التوكن في `localStorage` ويرسله مع كل طلب في هيدر `Authorization: Bearer <token>`
- الـ middleware يتحقق من صلاحية التوكن والصلاحيات قبل تنفيذ أي عملية

### حماية المسارات (Route Protection)

| المسار | الصلاحية المطلوبة |
|--------|-------------------|
| `/api/auth/login`, `/api/auth/register` | عام (بدون توكن) |
| `/api/services`, `/api/categories` | عام |
| `/api/gateways/public` | عام |
| `/api/settings/public/:key` | عام (إعدادات غير حساسة فقط) |
| `/api/v2` | API Key خاص بالمستخدم |
| `/api/orders` (GET/POST) | `requireAuth` — مستخدم مسجل |
| `/api/tickets`, `/api/coupons/redeem` | `requireAuth` |
| `/api/auth/me/:id`, `/api/auth/profile/:id` | `requireAuth` |
| `/api/stats`, `/api/settings/:key` | `requireAdmin` — أدمن فقط |
| `/api/gateways` (CRUD) | `requireAdmin` |
| `/api/providers`, `/api/coupons` (CRUD) | `requireAdmin` |
| `/api/notifications`, `/api/tickets` (admin) | `requireAdmin` |
| `/api/auth/users` (CRUD/ban) | `requireAdmin` |

### حماية إضافية

- **Rate Limiting** — تسجيل دخول الأدمن: 5 محاولات فاشلة = حظر 10 ساعات
- **CORS** — مقيد للدومين الخاص فقط (`APP_URL` + `localhost`)
- **bcrypt** — كلمات المرور مشفرة بـ bcrypt (10 rounds)
- **Input Whitelist** — حقول التعديل محددة مسبقاً (لا يمكن تعديل `role` أو `balance` من API المستخدم)
